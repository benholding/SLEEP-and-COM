...
packages <- c("xlsx", "plyr", "BayesFactor", "pastecs", "sjstats", "BEST", "apaStyle")
...
# check the output
speaking.summary <- summary(Speaking_best, ROPEm =c(-1,1), ROPEsd=c(-1, 1)) # haven't specified these so much
speaking.summary
...
#ROPE radius plot
speaking.meanDiff <- (Speaking_best$mu1 - Speaking_best$mu2)
plotAreaInROPE(speaking.meanDiff, maxROPEradius = 67)
...
# check the output
total_volume.summary <- summary(total_volume_best, ROPEm =c(-0.01,0.01)) # haven't specified these so much
total_volume.summary

#Creating a plot of the posterior distribution for the difference between groups
plot(total_volume_best) #the probability that the true value is smaller than zero = 95.5%

        #ROPE radius plot
volume.meanDiff <- (total_volume_best$mu1 - total_volume_best$mu2)
plotAreaInROPE(volume.meanDiff, maxROPEradius = 0.0013)
...
# check the output
variation_volume.summary <- summary(variation_volume_best, ROPEm =c(-0.01,0.01)) # haven't specified these so much
variation_volume.summary

#Creating a plot of the posterior distribution for the difference between groups
plot(variation_volume_best) #the probability that the true value is smaller than zero = 95.5%

#ROPE radius plot
variation_volume.meanDiff <- (variation_volume_best$mu1 - variation_volume_best$mu2)
plotAreaInROPE(variation_volume.meanDiff, maxROPEradius = 0.046)
