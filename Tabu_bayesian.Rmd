...
packages <- c("brms", "plyr", "dplyr", "data.table", "lubridate", "tidyr", "xlsx") #packages that are required
...
##HDI
Taboo.hdi <- hdi(brms::as.mcmc(tabu.both.model), credMass = 0.95)
hdi.Score_speaker <- Taboo.hdi[,2]
hdi.Score_guesser <- Taboo.hdi[,3]

##ROPE
taboo.speaker.mcmc <- as.mcmc(tabu.both.model, pars= "Speaker_SD", combine_chains = T)
taboo.guesser.mcmc <- as.mcmc(tabu.both.model, pars= "Guesser_SD", combine_chains = T)

plotAreaInROPE(taboo.speaker.mcmc, maxROPEradius = 0.25)
plotAreaInROPE(taboo.guesser.mcmc, maxROPEradius = 0.26)
...
#Analysis for attempts
```{r}
tabu.intercept.model2 <- brm(Correct ~ 1 + (1|pair), Tabu_with_rounds, sample_prior = T, cores = 4,family = poisson())

tabu.Speaker.model2 <- brm(total_attempts ~ Speaker_SD + (1|pair), Tabu_with_rounds,prior=priors_tabu, sample_prior = T, cores = 4,family = poisson())

tabu.guesser.model2 <- brm(total_attempts ~ Guesser_SD + (1|pair), Tabu_with_rounds,prior=priors_tabu, sample_prior = T, cores = 4,family = poisson())

tabu.both.model2 <- brm(total_attempts~ Speaker_SD+Guesser_SD + (1|pair), Tabu_with_rounds,prior=priors_tabu, sample_prior = T, cores = 4,family = poisson())

tabu.interaction.model2 <- brm(total_attempts ~ Speaker_SD*Guesser_SD + (1|pair), Tabu_with_rounds,prior=priors_tabu, sample_prior = T, cores = 4,family = poisson())

##### model comparison
tabu.intercept.waic2 <- WAIC(tabu.intercept.model2) #best model
tabu.speaker.waic2 <- WAIC(tabu.Speaker.model2)
tabu.guesser.waic <- WAIC(tabu.guesser.model2)
tabu.both.waic <- WAIC(tabu.both.model2) 
tabu.interaction.waic <- WAIC(tabu.interaction.model2)
compare_ic(tabu.intercept.waic2,tabu.speaker.waic2,tabu.guesser.waic,tabu.both.waic,tabu.interaction.waic) 
